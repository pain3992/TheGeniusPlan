	...
	if (GameInstance)
	{
		...
		for (FConstPlayerControllerIterator It = GetWorld()->GetPlayerControllerIterator(); It; ++It)
		{
			...
			if (PlayerController)
			{
				...
				if (PlayerState)
				{
					//for (auto PlayerData : GameInstance->SavedPlayerScores)
					//{
					//	if (PlayerData.PlayerName == PlayerState->GetPlayerName())
					//	{
							/* PlayerData.Score += PlayerState->GetScore();
							 PlayerData.GarnetCount += PlayerState->GetGarnetCount();
							 alreadyScore.Add(PlayerData.PlayerName);*/
		...
		//        if (PlayerState)
		//        {
		//           if(alreadyScore.Find(PlayerState->GetPlayerName()) == INDEX_NONE)
		//           {
		//               FPlayerScoreData ScoreData;
		//               ScoreData.PlayerName = PlayerState->GetPlayerName();
		//               ScoreData.Score = PlayerState->GetScore();
